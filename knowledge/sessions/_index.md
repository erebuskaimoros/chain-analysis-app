...
- **Graph filter browser validation (IN PROGRESS)**: A manual browser pass is still needed on Actor Tracker and Address Explorer to validate the new filter menu, per-transaction time trimming, and graph/action/stat synchronization under expansion and load-more flows.
